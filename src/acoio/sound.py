...
class Sound:
    ...
    def _getitem__indicies(self, slice_):
        ...
        new_end \
            = self._duration if j is None else j

        idx, jdx = \
            self.duration_to_index(new_start), \
            self.duration_to_index(new_end)
        return idx, jdx
    ...
    def _Frame(self, frame_duration: float =.08, frame_shift=.02):
        """
    ...
    def subtract(
        self, other: "Sound", α: float=5.0, β: float=.02,
        frame_duration: float=.08, frame_shift=.02, wtype='boxcar'
    ):
        """
